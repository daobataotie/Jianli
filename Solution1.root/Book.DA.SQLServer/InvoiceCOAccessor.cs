...
namespace Book.DA.SQLServer
{
    ...
    public partial class InvoiceCOAccessor : EntityAccessor, IInvoiceCOAccessor
    {
        ...
        public IList<Book.Model.InvoiceCO> SelectDateRangAndWhere(string costartid, string coendid, Model.Supplier SupplierStart, Model.Supplier SupplierEnd, DateTime? dateStart, DateTime? dateEnd, Model.Product productStart, Model.Product productEnd, string cusxoid, DateTime dateJHStart, DateTime dateJHEnd, int invoiceFlag, string InvoiceCGIdStart, string InvoiceCGIdEnd)
        {
            ...
            {
                if (!string.IsNullOrEmpty(InvoiceCGIdStart) && !string.IsNullOrEmpty(InvoiceCGIdEnd))
                    sql.Append(" and invoiceid in (select cod.InvoiceId from InvoiceCGDetail cgd right join InvoiceCODetail cod on cgd.InvoiceCODetailId=cod.InvoiceCODetailId where cgd.InvoiceId between '" + InvoiceCGIdStart + "' and '" + InvoiceCGIdEnd + "')");
                else
                    sql.Append(" and invoiceid in (select cod.InvoiceId from InvoiceCGDetail cgd right join InvoiceCODetail cod on cgd.InvoiceCODetailId=cod.InvoiceCODetailId where cgd.InvoiceId='" + (string.IsNullOrEmpty(InvoiceCGIdStart) ? InvoiceCGIdEnd : InvoiceCGIdStart) + "')");
            }
